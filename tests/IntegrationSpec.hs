    ...
    it "Can use `is` syntax" $ do
      compileAndRun
        [r|
                trait Person
                trait Human
                 type Person = (name: String, age: Int) is Person, Human
                |]
        `shouldReturn` ""
  describe "Traits" $ do
    ...
    it "Trait with both required properties and refinement" $ do
      compileAndRun
        [r|
                    ...
                    type Person = (name: String, age: Int) satisfies (age > 0)
    ...
    it "Struct with 'is' clause - trait with required properties (type mismatch)" $ do
      let prog =
            [r|
                    ...
                    type Person = (name: String, age: String) is Printable
    ...
    it "Struct with 'is' clause - trait with refinement (failing)" $ do
      let prog =
            [r|
                    ...
                    type Int = (x: Int) is PositiveNumber
    ...
    it "Can use generic Monad return with Some" $ do
      compileAndRun
        [r|
                ...
                |]
        `shouldReturn` "42\n"
    it "Can chain generic Monad bind with IO" $
      compileAndRun
        [r|
                    let main : IO<Unit> = do
                ...
                |]
        `shouldReturn` "4\n"
    it "Can use type to create polymorphic types" $ do
      compileAndRun
         [r|
                     type Person = Teacher(subject: String) | FootballPlayer(team: String) | Baby(age: Int)
