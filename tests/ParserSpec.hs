...
spec = do
  ...
  describe "Struct" $ do
    ...
    it "Should parse generic struct" $ do
      parseProgram "type Example<N: Number> = (content: N)" parserCompilerFlags
        `shouldBe` Right
    ...
    it "Can define" $
      parseProgram "type Teacher = ()" parserCompilerFlags
    ...
    it "Can define with is" $ do
      parseProgram "type Teacher = () is Person" parserCompilerFlags
    ...
    it "Can define with multiple is" $ do
      parseProgram "type Teacher = () is Person, Employee" parserCompilerFlags
  ...
  describe "Value structs" $ do
    it "Can define value struct without refinement" $ do
      parseProgram "value type PositiveInt = (num: Int)" parserCompilerFlags
    ...
    it "Can define value struct with refinement" $ do
      let result = parseProgram "value type EvenNumber = (num: Int) satisfies ((num % 2) == 0)" parserCompilerFlags
      case result of
    ...
    it "Can define value struct with is clause" $ do
      parseProgram "value type PositiveInt = (num: Int) is Printable" parserCompilerFlags
  ...
  describe "Type variants" $ do
    it "Should parse basic type with single constructor" $ do
      let result = parseProgram "type Person = Teacher(subject: String)" parserCompilerFlags
      case result of
        ...
        _ -> expectationFailure $ "Failed to parse basic type variant: " ++ show result
    it "Should parse type with multiple constructors" $ do
      let result = parseProgram "type Person = Teacher(subject: String) | FootballPlayer(team: String) | Baby(age: Int)" parserCompilerFlags
      case result of
        ...
        _ -> expectationFailure $ "Failed to parse type with multiple constructors: " ++ show result
    it "Should parse type with constructors having multiple fields" $ do
      let result = parseProgram "type Person = Teacher(subject: String, age: Int) | Student(name: String, grade: Int)" parserCompilerFlags
      case result of
        ...
        _ -> expectationFailure $ "Failed to parse type with multiple fields: " ++ show result
    it "Should parse type with constructors having no fields" $ do
      let result = parseProgram "type Option = Some(value: Int) | None()" parserCompilerFlags
      case result of
        ...
        _ -> expectationFailure $ "Failed to parse type with no fields: " ++ show result
