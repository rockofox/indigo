...
spec = do
  ...
  describe "Value structs" $ do
    it "Can cast to value struct without refinement" $ do
      result <-
        compileWithErrors
          [r|
                value type PositiveInt = (num: Int)
    ...
    it "Can cast to value struct with refinement (passing)" $ do
      result <-
        compileWithErrors
          [r|
                value type EvenNumber = (num: Int) satisfies ((num % 2) == 0)
  ...
  describe "Refinement types" $ do
    it "Should compile successfully when refinement passes" $ do
      result <-
        compileWithErrors
          [r|
                type Age = (value: Int) satisfies (value >= 0)
                let main : IO<Unit> = do
                    let a = Age{value: 5}
                end
                |]
    ...
    it "Should compile refinements with equality" $ do
      result <-
        compileWithErrors
          [r|
                type Person = (name: String) satisfies (name == "Alice")
                let main : IO<Unit> = do
                    let p = Person{name: "Alice"}
                end
                |]
      ...
      result <-
        compileWithErrors
          [r|
                ...
                type Teacher = (name: String, age: Int) satisfies ((notJohn name) && age > 18)
                let main : IO<Unit> = do
                    let hisAge = 19
  ...
  describe "Generics" $ do
    ...
    describe "Type parameter validation" $ do
      it "Should error on invalid type parameter in type is clause" $ do
        result <-
          compileWithErrors
            [r|
                    trait Optional<T>
                    type None = () is Optional<T>
                    let main : IO<Unit> = unsafePrint 42
                    |]
        case result of
          Left errors -> do
            ...
            any ("Invalid type parameter" `isInfixOf`) errorMessages
              `shouldBe` True
            any ("type None" `isInfixOf`) errorMessages
              `shouldBe` True
          Right _ -> expectationFailure "Expected compilation error for invalid type parameter in is clause"
      it "Should allow valid type parameter in type is clause" $ do
        result <-
          compileWithErrors
            [r|
                    trait Optional<T>
                    type Some<T> = (value: T) is Optional<T>
                    let main : IO<Unit> = unsafePrint 42
                    |]
        case result of
          Left errors -> expectationFailure $ "Expected successful compilation, got: " ++ show errors
          Right _ -> return ()
      it "Should error on invalid type parameter in type literal" $ do
        result <-
          compileWithErrors
            [r|
                    type Some<T> = (value: T)
        ...
        case result of
          Left errors -> do
            ...
            any ("Invalid type parameter" `isInfixOf`) errorMessages
              `shouldBe` True
            any ("type literal Some" `isInfixOf`) errorMessages
              `shouldBe` True
          Right _ -> expectationFailure "Expected compilation error for invalid type parameter in type literal"
      it "Should allow valid type parameter in type literal" $ do
        result <-
          compileWithErrors
            [r|
                    type Some<T> = (value: T)
      ...
      it "Should allow nested valid type parameter" $ do
        result <-
          compileWithErrors
            [r|
                    trait Optional<T>
                    type None = () is Optional<List<Int>>
                    let main : IO<Unit> = unsafePrint 42
                    |]
        case result of
          Left errors -> expectationFailure $ "Expected successful compilation, got: " ++ show errors
