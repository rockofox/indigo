...
spec = do
    describe "Addition" $ do
        it "Should compile 2+4" $ do
            compile [r|2+4|]
                `shouldReturn` [Label "main", StoreSideStack, Push 2, LStore "__op_a_0", Push 4, LStore "__op_b_0", LLoad "__op_a_0", LLoad "__op_b_0", Call "+", ClearSideStack, Exit]
        it "Should work properly with function calls" $ do
            compile
                [r|
                f x = x + 1
                let main => IO = unsafePrint (f 2) + 4|]
                `shouldReturn` [Label "f#0", StoreSideStack, LStore "x", LLoad "x", LStore "__op_a_1", Push 1, LStore "__op_b_1", LLoad "__op_a_1", LLoad "__op_b_1", Call "+", ClearSideStack, Ret, Label "main", StoreSideStack, Push 2, Call "f#0", LStore "__op_a_2", Push 4, LStore "__op_b_2", LLoad "__op_a_2", LLoad "__op_b_2", Call "+", Builtin Print, ClearSideStack, Exit]
    ...
    describe "Explicit casting" $ do
        it "Can cast from int to float" $ do
            compile [r|2 as Float|]
                `shouldReturn` [Label "main", StoreSideStack, Push 2, Push 0.0, Cast, ClearSideStack, Exit]
        it "Casts are compatible with binary operations" $ do
            compile [r|(2 as Float) + 4.0|]
                `shouldReturn` [Label "main", StoreSideStack, Push 2, Push 0.0, Cast, LStore "__op_a_0", Push 4.0, LStore "__op_b_0", LLoad "__op_a_0", LLoad "__op_b_0", Call "+", ClearSideStack, Exit]
